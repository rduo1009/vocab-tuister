...
func (v *value) goType(observations int, options *generateOptions, top bool) goType {
	...
	case distinctTypes == 2 && v.arrays > 0 && v.nulls > 0:
		elementGoType := v.arrayElements.goType(0, options, false)
		return goType{
	...
	case distinctTypes == 2 && v.objects > 0 && v.nulls > 0:
		...
		if hasUnparsableProperties {
			valueGoType := v.allObjectProperties.goType(0, options, false)
			return goType{
		...
		for _, property := range properties {
			...
			goType := v.objectProperties[property].goType(v.objects, options, false)
			var omitEmpty bool
			switch {
