...
A `huh.Form` is just a `ComponentModel`, so you can use it just as
you would any other [Bubble](https://github.com/charmbracelet/bubbles).

```go
type Model struct {
    form *huh.Form // huh.Form is just a ComponentModel
}
...
func (m Model) Update(msg tea.Msg) (ComponentModel, tea.Cmd) {
    // ...
...
Charm 热爱开源 • Charm loves open source • نحنُ نحب المصادر المفتوحة
