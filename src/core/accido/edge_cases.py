...
DERIVED_IRREGULAR_CHANGES: Final[
    ...
    "sum": lambda x: DictChanges(  # e.g. adsum, adesse, adfui, adfuturus
        ...
        replacements={
            ...
            "Vfutactinf   ": MultipleEndings(  # x for sum is ("", "fu", "futur")
                regular=f"{x[2]}um esse",
                second="fore",  # futurum esse, fore
            ),
        },
        additions={},
        # no passives, present participles
    ...
    "sum_preptc": lambda x: DictChanges(  # e.g. absum, abesse, afui, afuturus. x is (prefix, perfect_prefix, ppp_prefix) e.g. ("ab", "af", "afutur")
        ...
        replacements={
            ...
            "Vfutactipepl3": f"{x[0]}sunto",
            "Vfutactinf   ": (
                MultipleEndings(regular="adfuturum esse", second="adfore")
                if x[0] == "ad"
                else MultipleEndings(regular="afuturum esse", second="afore")
                if x[0] == "ab"
                else MultipleEndings(
                    regular=f"{x[2]}futurum esse", second=f"{x[2]}fore"
                )  # Fallback for other sum_preptc
            ),
        },
        additions={},
        # no passives
...
def find_derived_verb_changes(
    ...
    x_tuple = tuple(
        ...
        if pp
        else ""  # Ensure pp is not None
        for i, pp in enumerate(principal_parts)
    )

    return DERIVED_IRREGULAR_CHANGES[group](x_tuple)
