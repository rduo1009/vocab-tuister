...
@pytest.mark.integration
def test_cli_error_missing_config(snapshot, monkeypatch):
    server_url, vocab_list, session_config, cli_process = setup_tests(monkeypatch, 5502, "regular", "errormissing")
    try:
        sleep(5)
...
def test_cli_error_extra_config(snapshot, monkeypatch):
    server_url, vocab_list, session_config, cli_process = setup_tests(monkeypatch, 5503, "regular", "errorextra")
    try:
        sleep(5)
...
def test_cli_error_type1_config(snapshot, monkeypatch):
    server_url, vocab_list, session_config, cli_process = setup_tests(monkeypatch, 5504, "regular", "errortype1")
    try:
        sleep(5)
