...
def test_reader():
    l = read_vocab_file(Path("tests/lego_test/test_vocab_files/regular_list.txt"))
    assert l == VocabList([
...
def test_reader_with_s():
    l = read_vocab_file(Path("tests/lego_test/test_vocab_files/regular_with_s_list.txt"))
    assert l == VocabList([
        Verb("audio", "audire", "audivi", "auditus", meaning="hear"),
        Verb("capio", "capere", "cepi", meaning="take"),
        Noun("puella", "puellae", gender=Gender.FEMININE, meaning="girl"),
...
def test_multiple_meanings():
    l = read_vocab_file(Path("tests/lego_test/test_vocab_files/multiple_meanings_list.txt"))
...
def test_invalidpos():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_pos_list.txt"))
    assert str(error.value) == "Invalid part of speech: 'Error'"


def test_invalidlinefmt():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_linefmt_list.txt"))
    assert str(error.value) == "Invalid line format: 'error: error: error'"


def test_nopos():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/no_pos_list.txt"))
    assert str(error.value) == "Part of speech was not given"


def test_invalidverbfmt():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_verbfmt_list.txt"))
    assert str(error.value) == "Invalid verb format: 'hear: audio, audire, audivi, auditus, error, error, error'"


def test_invalidnounfmt():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_nounfmt_list.txt"))
    assert str(error.value) == "Invalid noun format: 'dog: canis, canis, error, error'"


def test_invalidadjfmt():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_adjfmt_list.txt"))
    assert str(error.value) == "Invalid adjective format: 'good: bonus, bona, bonum, error, error'"


def test_decl1():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_decl1_list.txt"))
    assert str(error.value) == "Invalid adjective declension: '3'"


def test_decl2():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_decl2_list.txt"))
    assert str(error.value) == "Invalid adjective declension: '4'"


def test_invalid_gender():
    with pytest.raises(InvalidVocabFileFormatError) as error:
        read_vocab_file(Path("tests/lego_test/test_vocab_files/invalid_gender_list.txt"))
