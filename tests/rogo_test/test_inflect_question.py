...
def test_inflect_question_adjective():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.main_answer in word.endings.values()
        ...
        for answer in q.answers:
            ...
            assert q.components is not None
            assert ending_components_original(q.components) in word.find(answer)

        assert q.prompt == str(word)
