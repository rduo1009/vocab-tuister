...
def test_multiplechoice_engtolat():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        q = output.mc_eng_to_lat

        assert type(q) is MultipleChoiceEngToLatQuestion
...
def test_multiplechoice_engtolat_adjective():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.answer in q.choices
        assert len(q.choices) == 3
        assert set(q.choices) == {"laetus", "ingens", "fortis"}
        assert (q.prompt, q.answer) in {("happy", "laetus"), ("large", "ingens"), ("strong", "fortis")}
...
def test_multiplechoice_engtolat_noun():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.answer in q.choices
        assert len(q.choices) == 3
        assert set(q.choices) == {"puella", "servus", "canis"}
        assert (q.prompt, q.answer) in {("girl", "puella"), ("slave", "servus"), ("dog", "canis")}
...
def test_multiplechoice_engtolat_pronoun():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.answer in q.choices
        assert len(q.choices) == 3
        assert set(q.choices) == {"hic", "ille", "qui"}
        assert (q.prompt, q.answer) in {("this", "hic"), ("that", "ille"), ("who", "qui")}
...
def test_multiplechoice_engtolat_verb():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.answer in q.choices
        assert len(q.choices) == 3
        assert set(q.choices) == {"doceo", "traho", "audio"}
        assert (q.prompt, q.answer) in {("I teach", "doceo"), ("I drag", "traho"), ("I hear", "audio")}
...
def test_multiplechoice_engtolat_regularword():
    ...
    for output in ask_question_without_sr(vocab_list, amount, session_config, settings):
        ...
        assert q.answer in q.choices
        assert len(q.choices) == 3
        assert set(q.choices) == {"in", "e", "post"}
        assert (q.prompt, q.answer) in {("in", "in"), ("out of", "e"), ("after", "post")}
